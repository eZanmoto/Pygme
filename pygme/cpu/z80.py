...
class Z80:
    ...
    DAA_NEG_INCS = [[0x00, 0xFA], [0xA0, 0X9A]]
    ...
    def _daa(self):
        ...
        a = self.a()
        if self.neg():
            inc = self.DAA_NEG_INCS[self.carry()][self.half_carry()]
        else:
            half_carry = bits.get(a, 3, 4) > 0x9 or self.half_carry()
            self.carry(bits.get(a, 7, 4) > 0x9 or self.carry())
            inc = half_carry*0x06 + self.carry()*0x60
        a_ = (a + inc) & 0xff
        self.a(a_)
        self.half_carry(0)
        self.zero(a_ == 0)
